...
function addMathBlock(start, asciiStart) {
    for (let i = 0; i < 26; i++) {
...
function checkHiddenUnicode(text) {
    ...
    const re = new RegExp('[\u{E0000}-\u{E007F}\u200B\u200C\u200D\u2060\uFEFF\u00AD' +
        '\u200E\u200F\u202A\u202B\u202C\u202D\u202E\u2066\u2067\u2068\u2069' +
        '\u061C\u034F]', 'gu');
    let m;
...
function checkBase64(text) {
    ...
    while ((m = re.exec(text)) !== null && hits.length < 20) {
        ...
        if (/data:\s*image\/|data:\s*[a-z]+\/[a-z+.-]+;base64,/i.test(before))
            continue;
...
function checkDangerousPaths(text) {
    const hits = [];
...
function checkPasswordAssignment(text) {
    const hits = [];
...
function checkDangerousShell(text) {
    const hits = [];
...
function checkSuspiciousBehavior(text) {
    const hits = [];
...
function checkI18nOverride(text) {
    const hits = [];
...
/** 对一段文本跑全部规则，返回命中列表 */
export function scanText(text, source, rules = buildDefaultEngine()) {
    if (!text)
        return [];
    const findings = [];
...
/** 评分：100 - 扣分 */
export function severityScore(findings) {
    if (!findings.length)
        return 100;
