...
function walk(dir: string, targets: ScanTarget[], depth: number): void {
  ...
  for (const e of entries) {
    ...
    if (e.isDirectory()) {
      if (!SKIP_DIRS.has(e.name)) walk(full, targets, depth + 1);
    } else if (e.isFile()) {
...
/** 扫描全部目标 */
export function scanAll(extraPaths: string[] = []): ScanTarget[] {
  ...
  for (const p of paths) {
    ...
    if (st.isDirectory()) walk(p, targets, 0);
    else scanFile(p, targets);
  }
  return targets;
}
